...
export class MasterDataController {
  constructor(containerId) {
    ...
    // Initial render from loaded storage
  ...
  _handleDataChangeInner(type) {
    if (type === 'weights') {
      ...
      if (data && data.length > 0) {
        ...
        if (!displayHeaders.includes("Line Number (Derived)")) {
          displayHeaders.push("Line Number (Derived)");
        }
        this.renderDumpPreview(data, displayHeaders);
        ...
        if (statusEl) {
          ...
          statusEl.textContent = `✓ Loaded ${data.length} records, ${uniqueLines.size} unique lines`;
          statusEl.style.color = "var(--green-ok)";
        ...
        <div id="pipingclass" class="tab-pane">
          <div style="margin-bottom: 0.5rem;">
            ...
            <div style="text-align:right;">
              <button id="btn-load-piping-csv" class="btn btn-primary btn-sm">Load Master for CSV Pipe size</button>
            </div>
          </div>
  ...
  bindEvents() {
    ...
    // Load Last ProcessMap — restores previously saved smart mapping and re-renders the UI
    ...
    if (loadProcessMapBtn) {
      loadProcessMapBtn.addEventListener("click", () => {
        ...
        this.logToLinelist("info", "✓ ProcessMap restored from saved config.");
      });
    }
  ...
  async handleUpload(file, type) {
    ...
    try {
      ...
      this.logToLinelist(
        "success",
        `✓ Detected ${result.data.length} rows, header at row ${result.detectedRow + 1}`,
      );
      ...
      if (type === "linelist") {
        ...
        this.logToLinelist(
          "info",
          `📋 Detected ${result.headers?.length || 0} columns`,
        );
        ...
        this.logToLinelist(
          "success",
          "✓ SmartProcessMap auto-fill complete (check dropdowns)",
        );
      } else if (type === "pipingclass") {
      ...
      this.logToLinelist(
        "success",
        `✓ Upload complete: ${result.data.length} rows loaded`,
      );
  ...
  async handleDumpUpload(file) {
    ...
    try {
      ...
      // Render preview
      // Add "Line Number" to headers for display if not present
      const displayHeaders = [...result.headers];
      if (!displayHeaders.includes("Line Number (Derived)")) {
        displayHeaders.push("Line Number (Derived)");
      }
      this.renderDumpPreview(dedupedData, displayHeaders);
      this.renderDumpConfig();
  ...
  renderDumpPreview(data, headers) {
    ...
    // Priority column names — try exact match first, then fuzzy (substring)
    const priorityCols = [
      "Reference of the element", "Reference", "Ref",
      "Name", "NAME", "Type",
      "Position", "POS", "POS WRT /*",
    ...
    // Fuzzy fallback: if nothing matched exactly (or only Line Number matched), try substring
    if (displayHeaders.length === 0 || (displayHeaders.length === 1 && displayHeaders[0].includes("Line"))) {
      const keyFragments = ["ref", "name", "type", "pos", "pipe", "line", "site"];
      const fuzzyMatches = headers.filter(h =>
        keyFragments.some(frag => h.toLowerCase().includes(frag))
      );
      if (fuzzyMatches.length > 0) {
        displayHeaders = fuzzyMatches;
      }
    }

    // Last resort: show all columns (truncated to first 8)
    if (displayHeaders.length === 0 || (displayHeaders.length === 1 && displayHeaders[0].includes("Line"))) {
      displayHeaders = headers.slice(0, 8);
    }

    // Always ensure Line Number / Line Number (Derived) columns are included
    if (!displayHeaders.includes("Line Number") && headers.includes("Line Number")) {
      displayHeaders.push("Line Number");
    }
