...
function buildMessageSquare(comp, cfg, isStub = false) {
  ...
  if (comp.type === 'SUPPORT') {
    return [
      'MESSAGE-SQUARE',
      `    SUPPORT, RefNo:=${comp.refNo}_${lastSegment(comp.supportGuid)}, ` +
      `SeqNo:${comp.seqNo}, ${comp.supportName}, ${comp.supportGuid}`
    ];
  }
...
function emitSupport(comp, stub, cfg) {
  ...
  lines.push(...buildMessageSquare(comp, cfg));
  lines.push('SUPPORT');
  lines.push(line(4, 'CO-ORDS   ', fmtCoord(comp.supportCoor, 0, cfg)));
  lines.push(line(4, '<SUPPORT_NAME>   ', comp.supportName));
  lines.push(line(4, '<SUPPORT_GUID>   ', comp.supportGuid));
  lines.push('');
  // 1mm stub PIPE
...
export function runStage2(components, logFn = () => {}) {
  ...
  for (const comp of components) {
    if (!cfg.fittingTypes.includes(comp.type)) {
