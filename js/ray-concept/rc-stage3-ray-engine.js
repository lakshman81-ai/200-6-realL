...
function buildFaces(components, cfg) {
  ...
  for (const c of components) {
    ...
    if ((c.type === 'TEE' || c.type === 'OLET') && c.cp) {
...
function injectBridge(fa, fb, pipelineRef, cfg) {
  ...
  if (fa._isOletDownstream || fb._isOletDownstream) return; // avoid infinite loop
  const connectedFace  = fa.connected ? fa : fb;
  const partnerFace    = fa.connected ? fb : fa;
  if (connectedFace.compType === 'OLET' && connectedFace.faceKey === 'ep1') {
    const ep2Face = _faces.find(f =>
      f.compRefNo === connectedFace.compRefNo &&
      f.faceKey   === 'ep2' &&
      !f.connected
    );
    if (ep2Face) {
      // Compute outgoing direction: opposite of incoming vector
      const incomingDir = vecNorm(vecSub(connectedFace.point, partnerFace.point));
      // ep2 shoots in same direction as the incoming ray (pipe continues through OLET)
      ep2Face._passthroughDir = incomingDir;
      _log?.('S3-P1', 'olet-passthrough-armed', connectedFace.compRefNo,
        { ep2Id: ep2Face.id, dir: incomingDir });
    }
  }
}
...
function rearmOletPassthroughs() {
  const oletEp1Faces = _faces.filter(
    f => f.compType === 'OLET' && f.faceKey === 'ep1' && f.connected
  );
  for (const ep1 of oletEp1Faces) {
    ...
    if (!partner) continue;
    // incomingDir: the vector from partner → ep1 (i.e., the pipe direction arriving at OLET)
    const incomingDir = vecNorm(vecSub(ep1.point, partner.point));
